...
Analysis pipeline for **PhIP-Seq data from the BC-Engl cohort**, built around development workflows from **`phiper`**.
...
- `Data/exist.csv`
- `Data/fold.csv`
- `Metadata/BC-Engl_metadata.csv`

Main output:

- `Data/BC-Engl.parquet`
...
Rscript src/R/03-render_phiper_reports.R \
  BASE_DIR=/absolute/path/to/BC-Engl/results \
...
Rscript src/R/03-render_phiper_reports.R \
  BASE_DIR=/absolute/path/to/BC-Engl/results \
  GROUP_COLS=group_test,Treatment,...
```
...
```bash
sbatch scripts/get_reports.sh /absolute/path/to/BC-Engl/results group_test
```

Or for several grouping variables:

```bash
sbatch scripts/get_reports.sh /absolute/path/to/BC-Engl/results group_test,Treatment,...
```

The Slurm job currently uses:

- job name: `BC-Engl-report`
- time: `00:20:00`
- CPUs: `1`
- memory: `10G`
